...
def remove_duplicates_set(s):
    ...
    for i in s:   
        if i not in set1:   # any time the letter is not in the set, it'll add it.
            str += i
            set1.add(i)
    return str
# Your unit tests
assert remove_duplicates_set("apple") == "aple"
assert remove_duplicates_set("applepineapple") == "aplein"
assert remove_duplicates_set('wahmoodidoodiloodi') == "wahmodil"
...
Write a function to calculate how many stones you've collected are actually gems. 
...
def gem_counting(stones, gems):
    count = 0
    for i in range(len(stones)):   #checks whether the letter is in the gem sequence.
        if stones[i] in gems:
            count += 1       # adds 1 if the letter is in the number.
    return count

# Your unit tests
assert gem_counting("ftT", "feq") == 1
assert gem_counting("HAmodihaBIb", "aHbOoB") == 5
assert gem_counting("Yufjehwqoq", "htuskYqOP") == 5
...
"""
def students_id(ids):
    set4 = set() # makes an empty set.
    for i in range(len(ids)):  #makes a for loop to check for each id to see if it's in the set.
        if ids[i] not in set4:
            set4.add(i)   # adds it to the set if it's not in there already.
    return len(set4)

# Your unit tests
assert students_id(['006', '008', '007']) == 3
assert students_id(['003', '004', '008', '007']) == 4
assert students_id(['006', '008', '004', '003', '007']) == 5
...
def students_id_occurrences(ids):
    ...
    id_dict = {}
    for i in ids:
        if ids not in [id_dict]:
            id_dict(i)
        return id_dict

# Your unit tests
print(students_id_occurrences(['002', '003', '001', '004', '012']))
